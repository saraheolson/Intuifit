...
async function createTestData() {
    try {
        const coach = await clerk_sdk_node_1.clerkClient.users.getUserList({
        ...
        const createdExercises = await Promise.all(sampleExercises.map(async (exercise) => {
        ...
        const createdWorkouts = await Promise.all(sampleWorkouts.map(async (workout) => {
